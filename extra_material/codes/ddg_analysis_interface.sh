...
then
    ...
    target_chain=$3
    ligand_chain=$4
    ROSETTA=/dors/meilerlab/apps/rosetta/rosetta-3.9/main/source/bin/residue_energy_breakdown.linuxgccrelease

    echo Computing per_residue ddg
    $ROSETTA -l $model_list -out:file:silent ${prefix}_ddg_score.out -out:pdb_gz 
    ...
    for res in `awk '{print $1}' ${prefix}_ddg_interface.res.lst|sort|uniq`; do echo $res `grep "^${res} " ${prefix}_${ligand_chain}_${target_chain}_interaction.ddg.sc | awk '{print $3}' | ~mendenjl/bin/ave_columns`; done | sed "s/${target_chain} / /g" > ${prefix}_chain${target_chain}.ave.res.ddg.sc
    for res in `awk '{print $2}' ${prefix}_ddg_interface.res.lst|sort|uniq`; do echo $res `grep " ${res} " ${prefix}_${ligand_chain}_${target_chain}_interaction.ddg.sc | awk '{print $3}' | ~mendenjl/bin/ave_columns`; done | sed "s/${ligand_chain} / /g" > ${prefix}_chain${ligand_chain}.ave.res.ddg.sc
	...
	set_residue_b_factor.scr ${prefix}_chain${target_chain}.ave.res.ddg.sc ${model}_${target_chain}.pdb > ${prefix}_chain${target_chain}.ddg.pdb
	set_residue_b_factor.scr ${prefix}_chain${ligand_chain}.ave.res.ddg.sc ${model}_${ligand_chain}.pdb > ${prefix}_chain${ligand_chain}.ddg.pdb
	#gzip ${model}
fi
